...
export async function reseller_modem_setting(req: Request, res: Response) {
    try {
        ...
        conn?.prepare("UPDATE modems SET user=?, password=? WHERE id=?").get(user,password,modem?.id)
        const client = await get_db();
        const collection_modems = client?.db?.collection("modems");
        collection_modems?.updateOne({"IMEI": modem?.IMEI}, {"$set": {"PROXY_USER": user, "PROXY_PASSWORD": password}})
